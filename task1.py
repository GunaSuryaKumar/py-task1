...
def delete_task():
    try:
        ...
        if the_value in tasks:
            ...
            list_update()
            the_cursor.execute('delete from tsks where title -?',(the_value))
    except:
        messagebox.showinfo('Error','No Task Selected . Cannot Delete')

def  delete_all_tasks():
    messagebox = messagebox.askeysno('Delete All' , 'Are You Sure?')
    if messagebox == True:
        while(len(tasks) != 0):
            tasks.pop()
        the_cursor.execute(delete_all_tasks)
        list_update()
...
if __name__ == "__main__":
    ...
    del_button.place(x = 30, у = 160)
    exit_button.place(x = 30, у = 200)
